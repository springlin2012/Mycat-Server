...
public class PartitionByDateTest {
    ...
    public void test() {
        ...
        partition.setsEndDate("2014-01-10");
        ...
        Assert.assertEquals(true, 2 == partition.calculate("2014-01-21"));
        Assert.assertEquals(true, 2 == partition.calculate("2014-01-30"));

        // 截至日期01.31 - 02.09 = 3/分片, 因超过截至时间后 10/天 会单独分一个片,
        // 轮询分片开始节点 = 截至日期 + 9, (分片0=2014-02-10至2014-02-19)
        // 如果 EndDate = 2014-01-31, 则不会出现分片3,只会有 0-2个分片
        Assert.assertEquals(true, 3 == partition.calculate("2014-01-31"));
        Assert.assertEquals(true, 3 == partition.calculate("2014-02-09"));

        Assert.assertEquals(true, 0 == partition.calculate("2014-02-19"));
        Assert.assertEquals(true, 1 == partition.calculate("2014-02-20"));
        Assert.assertEquals(true, 2 == partition.calculate("2014-02-30"));
